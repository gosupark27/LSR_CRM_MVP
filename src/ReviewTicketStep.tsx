...
import { DraftRepair, NewTicketInfo, Repair } from "./types";
...
export default function ReviewTicketStep({
  ...
  const customer = form.getValues().customer_info;
  const handleTextUpdate = () => {
    console.log("will send text to", customer.phone);
  };
  const handleEmailUpdate = () => {
    console.log("will send email to", customer.email);
  };

  const ticketDate = form.getValues().ticket_info.date_info;
  const ticketPayment = form.getValues().ticket_info.payment_info;
  const ticketItems = form.getValues().ticket_info.items;

  const calcItemTotal = (repairs: DraftRepair[]) => {
    return repairs.reduce((total, repair) => total + +repair.cost, 0);
  };
      ...
      <>
        ...
        >
          {item.repairs.map((repair) => {
            const isNote = repair.note !== "";
            const note = isNote ? `: ${repair.note}` : "";
            return (
              <>
                <Text fw={300}>{`${repair.rp_service}${note}`}</Text>
                <Text ta="right" fw={500}>{`$${repair.cost}`}</Text>
              </>
            );
          })}
        </Group>
        ...
        <Paper p="lg" shadow="sm" my="lg">
          ...
          <Stack my="xl" pt="xs">
            ...
            <Stack gap="xs">
              ...
              <Text c="dimmed">{ticketDate.dropoff_date}</Text>
            </Stack>
            <Stack gap="xs">
              <Text fw={500}>Pickup Date</Text>
              <Text c="dimmed">{ticketDate.pickup_date}</Text>
            </Stack>
            <Checkbox
              label={<Text fw={500}>Expedited service</Text>}
              checked={ticketDate.urgent}
              labelPosition="left"
              styles={{
          ...
          <Stack my="xl" pt="xs">
            ...
            <Group justify="space-between">
              <Text>Rush Fee</Text>
              <Text ta="right">{ticketPayment.fee}</Text>
            </Group>
            <Group justify="space-between">
              <Text>Deposit</Text>
              <Text ta="right">{ticketPayment.deposit}</Text>
            </Group>
            <Group justify="space-between">
            ...
            <Group justify="space-between">
              <Text>Balance</Text>
              <Text ta="right">{ticketPayment.balance}</Text>
            </Group>
          </Stack>
        </Paper>
        <Center>
